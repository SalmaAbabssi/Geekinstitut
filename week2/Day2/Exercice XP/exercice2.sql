...
select film_id , title , description, length , rental_rate from film where title = 'Trip Newton'

--  9 No luck finding your movie? Maybe you made a mistake spelling the name. Write a query to get the 
...
--12  Write a query which will join the data in the customer table and the payment table. You want to 
...
--12 You need to check your inventory. Write a query to get all the movies which are not in inventory.
select * from inventory
select f.film_id , f.title from film f where not exists (select * from inventory i where f.film_id = i.film_id)

--13  Write a query to find which city is in which country.
select * from city
...
-- 14 the amount and the date of payment ordered by the id of the staff member who sold them the dvd.
